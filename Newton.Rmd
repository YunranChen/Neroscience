...
rtgamma=function(size,shape_,rate_,a,b){
  ...
  x=qgamma(p = u*c_inv,shape=shape_,rate = rate_)
  return(x)
}
...
ind0=ceiling(theta0/((b-a)/ntheta))
p[ind0]=1/2
...
for (j in 1:n){
  p_x=map_dbl(theta,~dpois(x = Xs_bn[i,j],lambda = .x))
  int=((b-a)/2*sum(out$weights*map_dbl(xs_gq,~dpois(x = Xs_bn[i,j],lambda = .x))*p[ceiling(xs_gq/((b-a)/ntheta))]))*(1-p[ind0])+p[ind0]*dpois(x = Xs_bn[i,j],lambda = theta0)
...
for (col in 1:ncol(res)){
  res_f[-ind0,col]=res[ind0,col]*res[-ind0,col]
}
ggdf=res_f%>%as_tibble()%>%
  mutate(x=theta,true=true_f)%>%
  gather(data = .,key=estimation,value=p,-x)
plotme_f=ggplot(data=ggdf,mapping = aes(x=x,y=p,group=estimation))+geom_line(colour = "grey",alpha=0.7)+geom_line(data=ggdf%>%filter(estimation=="true"),mapping = aes(x=x,y=p))+ggtitle("Gamma(2,0.4)[0,10]+Delta_10+Poisson")
...
xs=sapply(1:ntheta,function(j){
  map_dbl(.x = Xss[,j],.f = function(xs_j){
    (b-a)/2*sum(out$weights*map_dbl(xs_gq,~dpois(x=round(xs_j),lambda = .x))*res[,j][ceiling(xs_gq/((b-a)/ntheta))])*(1-res[,j][ind0])+res[,j][ind0]*dpois(x = round(xs_j),lambda = theta0)
  })
...
true_xs=x_range(xs_vec = as.vector(Xs_bn),ntheta = ntheta,l = 0,u = Inf)
true=map_dbl(.x = true_xs,
             .f =function(xxs_i){ p=(b-a)/2*sum(out$weights*map_dbl(xs_gq,~dpois(x = round(xxs_i),lambda = .x))*(dtgamma(x_ = theta,shape_ = 2,rate_ = 0.4,a = a,b = b))[ceiling(xs_gq/((b-a)/ntheta))])*0.5+0.5*dpois(x = round(xxs_i),lambda = theta0)
             })

ggdf_p=data_frame(x=c(as.vector(Xss),true_xs)%>%round(.),p=c(as.vector(xs),true),estimation=c(paste0("est",rep(1:N,each=ntheta)),rep("true",ntheta)))
plotme_p=ggplot(data=ggdf_p,mapping = aes(x=x,y=p,group=estimation))+geom_line(colour = "grey",alpha=0.7)+geom_line(data=ggdf_p%>%filter(estimation=="true"),mapping = aes(x=x,y=p))+ggtitle("Gamma(2,0.4)[0,10]+Delta_10+Poisson")
...
```{r}
tic=proc.time()
PR_GPP(Xs_bn=Xs_bn,a=0,b=10,theta0 = 10,nGQ=20,ntheta=100)
toc=proc.time()
toc-tic
...
ppr=function(i,p_0,Xs_bn){
  ...
  pp=sapply(1:nP,function(ix){
    for (j in 1:n){
      p_x=map_dbl(theta,~dpois(x = Xs_bn_p[j,ix],lambda = .x))
      int=((b-a)/2*sum(out$weights*map_dbl(xs_gq,~dpois(x = Xs_bn_p[j,ix],lambda = .x))*p[ceiling(xs_gq/((b-a)/ntheta))])*(1-p[ind0]))+p[ind0]*dpois(x = Xs_bn_p[j,ix],lambda = theta0)
  p=(1-w[j])*p+w[j]*p_x*p/int
...
```{r}
tic=proc.time()
PPR_GPP(Xs_bn=Xs_bn,a=0,b=10,theta0 = 10,nGQ=20,ntheta=100)
toc=proc.time()
toc-tic
