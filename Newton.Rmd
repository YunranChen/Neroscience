...
```{r}
PR_GPP=function(Xs_bn,a,b,c,theta0,nGQ,ntheta){
...
p=rep(1/(b-a)/2,ntheta) #uniform
#p=dbeta(x = seq(0,1,length.out = ntheta),shape1 = c,shape2 = c)/(b-a)/2
inds=ceiling(theta0/((b-a)/ntheta))
p[inds]=c(0.25,0.25)
...
for (j in 1:n){
  p_x=map_dbl(theta,~dpois(x = Xs_bn[i,j],lambda = .x))
  int=((b-a)/2*sum(out$weights*map_dbl(xs_gq,~dpois(x = Xs_bn[i,j],lambda = .x))*p[ceiling(xs_gq/((b-a)/ntheta))]))*(1-sum(p[inds]))+p[inds[1]]*dpois(x = Xs_bn[i,j],lambda = theta0[1])+p[inds[2]]*dpois(x = Xs_bn[i,j],lambda = theta0[2])
  p=(1-w[j])*p+w[j]*p_x*p/int
}
  return(p)
}
